...
def dispatch(h,m,p,q,a,db):
 ...
 if root=='organizations':
  ...
  if m=='POST':
   ...
   try:
    with db.tx() as c:
     c.execute('INSERT INTO organizations VALUES(?,?,?,?,?)',(oid,name,'active',n,n));c.execute('INSERT INTO workspaces(workspace_id,name,status,created_at,updated_at,organization_id) VALUES(?,?,\'active\',?,?,?)',(wid,name+'工作区',n,n,oid));c.execute('INSERT INTO workspace_configs VALUES(?,0,\'[]\',?,?)',(wid,n,a['user_id']));c.execute('INSERT INTO teams VALUES(?,?,?,?,?,?,?)',(tid,oid,wid,'默认团队','active',n,n))
 ...
 if root=='dashboard' and m=='GET':
  ...
  return h.sendj({'ok':True,'counts':counts,'channelDistribution':channel,'deviceStatus':status,'recentAudit':recent,'database':db.quick_check(),'schemaVersion':207,'version':'207.0.0'}) or True
 # teams
 if root=='teams':
  if m=='GET':
  ...
  if m=='POST':
   ...
   with db.tx() as c:c.execute('INSERT INTO workspaces(workspace_id,name,status,created_at,updated_at,organization_id) VALUES(?,?,\'active\',?,?,?)',(wid,str(b.get('workspaceName') or name),n,n,org));c.execute('INSERT INTO workspace_configs VALUES(?,0,\'[]\',?,?)',(wid,n,a['user_id']));c.execute('INSERT INTO teams VALUES(?,?,?,?,?,?,?)',(tid,org,wid,name,'active',n,n))
  ...
  if ident and action=='members':
   ...
   if m=='POST':
    ...
    with db.tx() as c:
     ...
     c.execute("INSERT INTO team_memberships VALUES(?,?,?,'active') ON CONFLICT(user_id,team_id,role_id) DO UPDATE SET status='active'",(uid,ident,role))
 ...
 if root=='users':
  ...
  if m=='POST':
   ...
   with db.tx() as c:
    ...
    try:
     c.execute('INSERT INTO users VALUES(?,?,?,?,?,?,?)',(uid,org,ln,str(b.get('displayName') or ln),'active',n,n))
     c.execute('INSERT INTO user_credentials VALUES(?,?,?,?,?,?)',(uid,ph,salt,it,1,n))
     c.execute("INSERT INTO team_memberships VALUES(?,?,?,'active')",(uid,team,role))
     if role in ('role_orgadmin','role_superadmin','role_auditor'):
      c.execute("INSERT INTO organization_memberships VALUES(?,?,?,'active')",(uid,org,role))
    except Exception:
 ...
 if root=='accounts':
  ...
  if m=='POST':
   ...
   b=h.body();ch=str(b.get('channel') or '').lower();iv=str(b.get('identity') or '').strip();team=str(b.get('teamId') or '')
   if ch not in CHANNELS or not iv:return bad(h,'INVALID_ACCOUNT','渠道或身份无效')
   with db.tx() as c:
 ...
 if root=='diagnostics' and m=='GET':
  ...
  path=str(db.path);return h.sendj({'ok':True,'version':'207.0.0','schemaVersion':207,'quickCheck':db.quick_check(),'databasePath':path,'databaseBytes':os.path.getsize(path) if os.path.exists(path) else 0,'walExists':os.path.exists(path+'-wal'),'shmExists':os.path.exists(path+'-shm'),'foreignKeyIssues':fk,'tableCounts':counts}) or True
