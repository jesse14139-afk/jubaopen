...
def register_device(db,a,p):
 ...
 with db.tx() as c:
  ...
  if not c.execute('SELECT 1 FROM device_assignments WHERE device_id=? AND released_at IS NULL',(did,)).fetchone():c.execute('INSERT INTO device_assignments VALUES(?,?,?,?,?,NULL)',('asn_'+did,did,a['user_id'],team,t))
