...
VERSION='207.0.0';DB=None;ADMIN=open(os.path.join(ROOT,'admin_v207.html'),'rb').read()
READ_PERM={'/api/v207/contacts':'contact.read','/api/v207/changes':'contact.read','/api/v207/admin/graph':'organization.read'}
class H(BaseHTTPRequestHandler):
 server_version='Jubaopen/207.0.0'
 def log_message(self,fmt,*args):sys.stderr.write('%s %s\n'%(self.address_string(),fmt%args))
 def sendj(self,d,s=200):
  b=json.dumps(d,ensure_ascii=False,separators=(',',':')).encode();self.send_response(s);self.send_header('Content-Type','application/json; charset=utf-8');self.send_header('Content-Length',str(len(b)));self.send_header('Access-Control-Allow-Origin','*');self.send_header('Access-Control-Allow-Headers','Authorization,Content-Type');self.send_header('Access-Control-Allow-Methods','GET,POST,PATCH,DELETE,OPTIONS');self.send_header('Cache-Control','no-store');self.end_headers();self.wfile.write(b)
 def body(self):
 ...
 def do_OPTIONS(self):self.send_response(204);self.send_header('Access-Control-Allow-Origin','*');self.send_header('Access-Control-Allow-Headers','Authorization,Content-Type');self.send_header('Access-Control-Allow-Methods','GET,POST,PATCH,DELETE,OPTIONS');self.end_headers()
 ...
 def route(self,m):
  ...
  if p in ('/admin','/admin/'):
   self.send_response(200);self.send_header('Content-Type','text/html; charset=utf-8');self.send_header('Content-Length',str(len(ADMIN)));self.end_headers();return self.wfile.write(ADMIN)
  ...
  if p.startswith('/api/v207/admin/') and admin_dispatch(self,m,p,q,a,DB):return
 ...
 def channel_account(self,a):
  ...
  b=self.body();ch=str(b.get('channel') or '').lower();identity=canonical(b.get('identity'));team=str(b.get('teamId') or '')
  if ch not in ('whatsapp','instagram','facebook','messenger','telegram') or not identity or not team:return self.sendj({'ok':False,'code':'INVALID_ACCOUNT'},400)
  with DB.tx() as c:
   ...
   n=now_ms();aid='acc_'+uuid.uuid4().hex;iid='idn_'+uuid.uuid4().hex;sid='src_'+uuid.uuid4().hex
   c.execute('INSERT INTO channel_accounts VALUES(?,?,?,?,?,?,?)',(aid,a['organization_id'],ch,str(b.get('displayName') or identity),'active',n,n));c.execute('INSERT INTO channel_account_identities VALUES(?,?,?,?,?,?)',(iid,aid,str(b.get('identityType') or 'login'),str(b.get('identity')),identity,n));c.execute('INSERT INTO team_channel_accounts VALUES(?,?,?,?)',(team,aid,a['user_id'],n));c.execute("INSERT INTO sources(source_id,workspace_id,channel,account_identity,source_name,status,first_seen_at,last_seen_at,metadata_json,organization_id,team_id,channel_account_id) VALUES(?,?,?,?,?,'active',?,?,'{}',?,?,?)",(sid,t['workspace_id'],ch,str(b.get('identity')),str(b.get('sourceName') or b.get('displayName') or identity),n,n,a['organization_id'],team,aid))
 ...
 def create_team(self,a):
  ...
  with DB.tx() as c:c.execute('INSERT INTO workspaces(workspace_id,name,status,created_at,updated_at,organization_id) VALUES(?,?,\'active\',?,?,?)',(wid,name,n,n,a['organization_id']));c.execute('INSERT INTO workspace_configs VALUES(?,0,\'[]\',?,?)',(wid,n,a['user_id']));c.execute('INSERT INTO teams VALUES(?,?,?,?,?,?,?)',(tid,a['organization_id'],wid,name,'active',n,n))
 ...
 def create_user(self,a):
  ...
  if len(pwd)<10 or not loginname:return self.sendj({'ok':False,'code':'WEAK_OR_INVALID_CREDENTIAL'},400)
  h,salt,it=hash_password(pwd)
  try:
   with DB.tx() as c:c.execute('INSERT INTO users VALUES(?,?,?,?,?,?,?)',(uid,a['organization_id'],loginname,str(b.get('displayName') or loginname),'active',n,n));c.execute('INSERT INTO user_credentials VALUES(?,?,?,?,?,?)',(uid,h,salt,it,1,n));c.execute('INSERT INTO team_memberships VALUES(?,?,?,\'active\')',(uid,team,role))
