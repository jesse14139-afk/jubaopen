...
def dispatch(h,m,p,q,a,db):
 ...
 if root=='dashboard' and m=='GET':
  ...
  return h.sendj({'ok':True,'counts':counts,'channelDistribution':channel,'deviceStatus':status,'recentAudit':recent,'database':db.quick_check(),'schemaVersion':207,'version':'207.3.0-dev'}) or True
 # teams
 if root=='teams':
  if m=='GET':
 ...
 if root=='workspaces':
  ...
  if m=='POST' and not ident:
   ...
   audit(db,a,'create','workspace',wid,{},b,wid=wid);return h.sendj({'ok':True,'workspaceId':wid},201) or True
  if ident and m=='PATCH':
   if not manage(db,a,'team.manage',org):return bad(h,'FORBIDDEN','无工作区管理权限',403)
   b=h.body()
   with db.tx() as c:
 ...
 if root=='sessions' and m=='GET':
  ...
  pno,size,off=page(q)
  with db.tx(False) as c:total=c.execute('SELECT count(*) FROM user_sessions s JOIN users u ON u.user_id=s.user_id WHERE u.organization_id=?',(org,)).fetchone()[0];its=rows(c,'SELECT s.*,u.display_name user_name FROM user_sessions s JOIN users u ON u.user_id=s.user_id WHERE u.organization_id=? ORDER BY s.created_at DESC LIMIT ? OFFSET ?',(org,size,off))
  return h.sendj(pack(its,total,pno,size)) or True
 if root=='sessions' and ident and m=='DELETE':
  if not manage(db,a,'user.disable',org):return bad(h,'FORBIDDEN','无会话撤销权限',403)
  with db.tx() as c:
   old=one(c,'SELECT s.*,u.organization_id FROM user_sessions s JOIN users u ON u.user_id=s.user_id WHERE s.session_id=?',(ident,))
   if not old or old['organization_id']!=org:return bad(h,'NOT_FOUND','会话不存在',404)
   c.execute('DELETE FROM user_sessions WHERE session_id=?',(ident,))
   audit_in_tx(c,a,'revoke','session',ident,old,{},'管理员撤销会话')
 ...
 if root=='tags' and m=='POST':
  ...
  b=h.body();wid=str(b.get('workspace') or '').strip();name=str(b.get('name') or '').strip();kind=str(b.get('kind') or 'profile')
  if not wid or not name:return bad(h,'INVALID_INPUT','工作区和名称不能为空')
  if kind not in ('status','profile'):return bad(h,'INVALID_KIND','标签类型必须为 status 或 profile')
  ...
  try:
   ...
   audit(db,a,'create','tag',tid,{},item,wid);return h.sendj({'ok':True,'item':item},201) or True
 ...
 if root=='tags' and ident and m=='DELETE':
  ...
  with db.tx() as c:
   ...
   audit(db,a,'delete','tag',ident,old,{},old['workspace_id'])
  return h.sendj({'ok':True}) or True
 ...
 if root=='field-definitions' and m=='POST':
  ...
  try:
   ...
   audit(db,a,'create','field_definition',key,{},item,wid);return h.sendj({'ok':True,'item':item},201) or True
 ...
 if root=='field-definitions' and ident and m=='DELETE':
  ...
  with db.tx() as c:
   ...
   audit(db,a,'delete','field_definition',ident,old,{},wid)
  return h.sendj({'ok':True}) or True
 # diagnostics
 if root=='diagnostics' and m=='GET':
  ...
  path=str(db.path);return h.sendj({'ok':True,'version':'207.3.0-dev','schemaVersion':207,'quickCheck':db.quick_check(),'databasePath':path,'databaseBytes':os.path.getsize(path) if os.path.exists(path) else 0,'walExists':os.path.exists(path+'-wal'),'shmExists':os.path.exists(path+'-shm'),'foreignKeyIssues':fk,'tableCounts':counts}) or True
