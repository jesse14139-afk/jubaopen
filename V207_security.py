...
def login(db,organization_id,login_name,password,ip='',ua=''):
 with db.tx() as c:
  ...
  token=secrets.token_urlsafe(40);sid='ses_'+secrets.token_hex(16);t=now_ms();c.execute('INSERT INTO sessions VALUES(?,?,?,?,?,?,?,?,NULL)',(sid,u['user_id'],hashlib.sha256(token.encode()).hexdigest(),t+SESSION_MS,ip,ua,t,t))
