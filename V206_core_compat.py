...
def register_device(db,p):
 ...
 with db.tx() as c:
  ...
  t=now_ms();c.execute("INSERT INTO devices(device_id,workspace_id,installation_id,device_name,computer_name,status,created_at,last_seen_at,metadata_json) VALUES(?,?,?,?,?,'active',?,?,?) ON CONFLICT(device_id) DO UPDATE SET device_name=excluded.device_name,computer_name=excluded.computer_name,last_seen_at=excluded.last_seen_at,metadata_json=excluded.metadata_json",(did,wid,iid,name,computer,t,t,dumps(p.get("metadata") or {})))
...
def delete_preview(db,wid,did,cid,version):
 with db.tx() as c:
  ...
  x="del_"+secrets.token_hex(16);impact=hash_obj({"contactId":cid,"version":version});exp=now_ms()+300000;c.execute("INSERT INTO delete_confirmations(confirmation_id,workspace_id,entity_type,entity_id,expected_version,impact_hash,expires_at,used_at,created_by) VALUES(?,?,?,?,?,?,?,?,?)",(x,wid,"contact",cid,int(version),impact,exp,None,did));return ok(confirmationId=x,expectedVersion=int(version),impact={"contactId":cid,"softDelete":True},expiresAt=exp)
...
def save_tag(db,wid,p):
 ...
 t=now_ms()
 with db.tx() as c:
  try:c.execute("INSERT INTO tags(tag_id,workspace_id,name,color,category,sort_order,enabled,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(tag_id) DO UPDATE SET name=excluded.name,color=excluded.color,category=excluded.category,sort_order=excluded.sort_order,enabled=excluded.enabled,updated_at=excluded.updated_at",(tid,wid,name,str(p.get('color') or '#64748b')[:20],str(p.get('category') or '')[:80],int(p.get('sort_order') or 0),1 if p.get('enabled',True) else 0,t,t))
