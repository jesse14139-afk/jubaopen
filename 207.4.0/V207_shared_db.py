...
from V207_version import SCHEMA_VERSION, PRODUCT_VERSION
...
def ensure_v207(c,admin_password=None,fresh=False):
 ...
 for table,fields in {'workspaces':[('organization_id','TEXT')],'sources':[('organization_id','TEXT'),('team_id','TEXT'),('channel_account_id','TEXT')],'devices':[('organization_id','TEXT'),('team_id','TEXT'),('assigned_user_id','TEXT'),('system_device_name','TEXT'),('system_computer_name','TEXT'),('user_device_alias','TEXT'),('user_computer_alias','TEXT'),('admin_device_name','TEXT'),('admin_computer_name','TEXT')],'contacts':[('organization_id','TEXT'),('team_id','TEXT')],'events':[('user_id','TEXT') ]}.items():
  for col,decl in fields:_add(c,table,col,decl)
 # 新库可以创建明确的默认租户；已有库绝不自动推断或重归属历史业务数据。
 if not fresh:
 ...
 if fresh:
  c.execute("UPDATE workspaces SET organization_id='org_default' WHERE organization_id IS NULL")
  c.execute("UPDATE sources SET organization_id='org_default',team_id='team_default' WHERE organization_id IS NULL OR team_id IS NULL")
  c.execute("UPDATE devices SET organization_id='org_default',team_id='team_default',system_device_name=COALESCE(system_device_name,device_name),system_computer_name=COALESCE(system_computer_name,computer_name) WHERE organization_id IS NULL OR team_id IS NULL")
  c.execute("UPDATE contacts SET organization_id='org_default',team_id='team_default' WHERE organization_id IS NULL OR team_id IS NULL")
...
def validate_registered_migration_checksums(path):
 ...
 try:
  ...
  if bad:raise RuntimeError('迁移校验和不匹配，已阻止启动: '+', '.join(bad))
 finally:c.close()
...
class Database:
 ...
 def initialize(self,admin_password=None):
  ...
  try:
   ...
   ensure_v207(c,admin_password,fresh=fresh);ensure_v207_1(c);ensure_v207_101(c);ensure_v207_200(c);ensure_v207_300(c);ensure_v207_301(c)
   c.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('product_version',?)",(PRODUCT_VERSION,))
