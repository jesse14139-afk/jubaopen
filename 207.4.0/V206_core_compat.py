# -*- coding: utf-8 -*-
"""V206 兼容层，运行于 Schema 207（V207.3.0）。

职责：为 ``V207_center_service`` 提供 ``apply_event / list_contacts / get_contact /
changes / delete_preview / get_workspace / update_workspace / list_sources / list_tags /
save_tag / delete_tag / list_field_definitions / save_field_definition /
delete_field_definition / admin_contact_lifecycle / admin_logs / admin_schema`` 等
兼容函数；保持 V205/V206 客户端事件协议不变。

注意：
* Schema 已从 205 升级到 207。``workspaces`` / ``sources`` / ``devices`` /
  ``contacts`` 等表均补齐了 ``organization_id``、``team_id``、``device`` 的
  系统/用户/管理员命名、``sources`` 的 ``platform_id/source_type/...`` 等列，
  本文件在写入时同步补齐，读取时合并到公开字典。
* V207 服务端已经将 ``/api/v205/``、``/api/v206/`` 入口返回 426，外部 V205/V206
  客户端事件不会触发本层；但中心服务仍按 ``apply_event`` 内部路径处理与历史同
  协议的回放/审计，因此本层需要继续遵循 V205 业务约束。
"""
...
def public_device(r):
 ...
 d=dict(r)
 d["metadata"]=loads(d.pop("metadata_json"),{}) or {}
 for k in ("device_name","computer_name","installation_id","status","first_seen_at","last_seen_at","device_id","organization_id","team_id","assigned_user_id","system_device_name","system_computer_name","user_device_alias","user_computer_alias","admin_device_name","admin_computer_name"):
  if k not in d: d.setdefault(k,None)
 return d
...
def public_contact(c,r):
 if not r:return None
 d=dict(r)
 d["observed"]=loads(d.pop("observed_json"),{}) or {}
 d["business"]=loads(d.pop("business_json"),{}) or {}
 ...
 s=c.execute("SELECT * FROM sources WHERE source_id=?",(d["source_id"],)).fetchone()
 dev=c.execute("SELECT * FROM devices WHERE device_id=?",(d["last_observed_device_id"],)).fetchone() if d.get("last_observed_device_id") else None
...
def register_device(db,p):
 ...
 computer=str(p.get("computer_name") or "").strip()[:120]
 sys_alias=str(p.get("system_device_name") or name)[:120]
 sys_comp=str(p.get("system_computer_name") or computer)[:120]
 ...
 with db.tx() as c:
  ...
  t=now_ms()
  c.execute(
   "INSERT INTO devices(device_id,workspace_id,installation_id,device_name,computer_name,status,first_seen_at,last_seen_at,metadata_json,organization_id,team_id,system_device_name,system_computer_name,user_device_alias,user_computer_alias,assigned_user_id) "
   "VALUES(?,?,?,?,?,'active',?,?,?,?,?,?,?,?,?,NULL) "
...
def context(db,wid,did,sid=None):
 with db.tx(False) as c:
  ws=workspace(c,wid)
  dev=c.execute("SELECT * FROM devices WHERE device_id=? AND workspace_id=?",(did,wid)).fetchone()
  src=c.execute("SELECT * FROM sources WHERE source_id=? AND workspace_id=?",(sid,wid)).fetchone() if sid else None
...
def add_change(c,wid,typ,eid,op,ver,sid,did,payload,event):
 c.execute("INSERT INTO changes(workspace_id,entity_type,entity_id,operation,version,source_id,device_id,changed_at,payload_json,event_id) VALUES(?,?,?,?,?,?,?,?,?,?)",(wid,typ,eid,op,ver,sid,did,now_ms(),dumps(payload),event))
...
def apply_event(db,e):
 ...
 eid=str(e["eventId"]);wid=str(e["workspaceId"]);did=str(e["deviceId"]);sid=str(e.get("sourceId") or "")
 p=e.get("payload") or {}
 ph=hash_obj({k:e.get(k) for k in ("workspaceId","deviceId","installationId","sourceId","entityType","entityId","operation","baseVersion")} | {"payload":p})
 with db.tx() as c:
  ...
  else:result,status=_contact(c,e,dev,src,org_id)
  stored=dict(result);stored["_http"]=status
  c.execute("UPDATE events SET status=?,result_version=?,error_code=?,applied_at=?,result_json=? WHERE event_id=?",
    ("applied" if result.get("ok") else "rejected",result.get("version"),result.get("code"),now_ms(),dumps(stored),eid))
  return result,status

def _contact(c,e,dev,src,org_id):
 ...
 if op=="contact.observe":
  ...
  row=c.execute("SELECT * FROM contacts WHERE workspace_id=? AND source_id=? AND channel=? AND external_contact_id=?",(wid,sid,ch,ext)).fetchone()
  obs={k:(stable(p.get(k)) if k=="observed" else p.get(k)) for k in OBS_KEYS if k in p};oh=hash_obj(obs)
  t=now_ms();touch_relation(c,wid,sid,did,op,True)
  c.execute("UPDATE sources SET last_seen_at=?,last_device_id=?,updated_at=? WHERE source_id=?",(t,did,t,sid))
  if not row:
   cid=uid("ct")
   c.execute(
   ...
   if row["observation_hash"]==oh:
    device_changed=str(row["last_observed_device_id"] or "")!=str(did)
    c.execute("UPDATE contacts SET last_observed_device_id=?,last_observed_at=?,last_seen_at=?,updated_at=? WHERE contact_id=?",(did,t,t,t,cid))
    data=public_contact(c,c.execute("SELECT * FROM contacts WHERE contact_id=?",(cid,)).fetchone())
   ...
   ver=row["version"]+1
   c.execute(
    "UPDATE contacts SET display_name=?,observed_phone=?,avatar_url=?,observed_json=?,observation_hash=?,last_observed_device_id=?,last_observed_at=?,version=?,updated_at=?,last_seen_at=? WHERE contact_id=?",
    (str(p.get("displayName") or ""),str(p.get("observedPhone") or ""),str(p.get("avatarUrl") or ""),dumps(stable(p.get("observed") or {})),oh,did,t,ver,t,t,cid))
   action="observe"
  data=public_contact(c,c.execute("SELECT * FROM contacts WHERE contact_id=?",(cid,)).fetchone())
  add_change(c,wid,"contact",cid,action,ver,sid,did,data,eid)
  return dict(ok=True,eventId=eid,entityId=cid,version=ver,contact=data),200
 ...
 if op=="contact.patch":
  ...
  for key,value in custom.items():
   ...
   c.execute("INSERT INTO contact_custom_values(contact_id,field_key,value_json,updated_at,updated_by) VALUES(?,?,?,?,?) ON CONFLICT(contact_id,field_key) DO UPDATE SET value_json=excluded.value_json,updated_at=excluded.updated_at,updated_by=excluded.updated_by",(cid,key,dumps(value),t,did))
  sets.extend(["version=?","updated_at=?"]);vals.extend([ver,t,cid])
  ...
  c.execute("UPDATE delete_confirmations SET used_at=? WHERE confirmation_id=?",(t,q["confirmation_id"]))
  c.execute("UPDATE contacts SET deleted_at=?,deleted_by=?,delete_reason=?,version=?,updated_at=? WHERE contact_id=?",(t,did,str(p.get("reason") or ""),ver,t,cid))
  action="delete"
 elif op=="contact.restore":
  if row["deleted_at"] is None:return err("NOT_DELETED","联系人未被删除",409)
  c.execute("UPDATE contacts SET deleted_at=NULL,deleted_by=NULL,delete_reason=NULL,version=?,updated_at=? WHERE contact_id=?",(ver,t,cid))
  action="restore"
...
def delete_preview(db,wid,did,cid,version):
 with db.tx() as c:
  ...
  x="del_"+secrets.token_hex(16);impact=hash_obj({"contactId":cid,"version":version});exp=now_ms()+300000
  c.execute("INSERT INTO delete_confirmations(confirmation_id,workspace_id,entity_type,entity_id,expected_version,impact_hash,expires_at,used_at,created_by) VALUES(?,?,?,?,?,?,?,?,?)",(x,wid,"contact",cid,int(version),impact,exp,None,did))
...
def list_contacts(db,qs):
 ...
 with db.tx(False) as c:
  rows=c.execute("SELECT c.rowid AS _cursor,c.* FROM contacts c WHERE "+" AND ".join(where)+" ORDER BY c.rowid LIMIT ?",args+[limit+1]).fetchall();more=len(rows)>limit;rows=rows[:limit]
  items=[public_contact(c,r) for r in rows];nxt=rows[-1]["_cursor"] if rows else after
  return ok(items=items,page={"limit":limit,"next_cursor":nxt,"has_more":more})
...
def set_device_status(db,wid,did,status):
 ...
 with db.tx() as c:
  ...
  c.execute("UPDATE devices SET status=?,last_seen_at=? WHERE workspace_id=? AND device_id=?",(status,now_ms(),wid,did))
  return ok(device=public_device(c.execute("SELECT * FROM devices WHERE device_id=?",(did,)).fetchone()))
...
def save_tag(db,wid,p):
 ...
 kind=str(p.get('tag_kind') or 'portrait').strip()
 if kind not in ('status','portrait'):return err('INVALID_TAG_KIND','tag_kind 必须是 status 或 portrait')
 t=now_ms()
 with db.tx() as c:
  try:c.execute("INSERT INTO tags(tag_id,workspace_id,name,color,category,tag_kind,sort_order,enabled,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(tag_id) DO UPDATE SET name=excluded.name,color=excluded.color,category=excluded.category,tag_kind=excluded.tag_kind,sort_order=excluded.sort_order,enabled=excluded.enabled,updated_at=excluded.updated_at",(tid,wid,name,str(p.get('color') or '#64748b')[:20],str(p.get('category') or '')[:80],kind,int(p.get('sort_order') or 0),1 if p.get('enabled',True) else 0,t,t))
...
def save_field_definition(db,wid,p):
 ...
 t=now_ms()
 with db.tx() as c:
  c.execute("INSERT INTO custom_field_definitions(workspace_id,field_key,label,field_type,sort_order,enabled,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(workspace_id,field_key) DO UPDATE SET label=excluded.label,field_type=excluded.field_type,sort_order=excluded.sort_order,enabled=excluded.enabled,updated_at=excluded.updated_at",(wid,key,str(p.get('label') or key)[:120],typ,int(p.get('sort_order') or 0),1 if p.get('enabled',True) else 0,t,t))
  return ok(field=dict(c.execute("SELECT * FROM custom_field_definitions WHERE workspace_id=? AND field_key=?",(wid,key)).fetchone()))
...
def set_contact_tags(db,wid,cid,tag_ids,did,base_version):
 ...
 with db.tx() as c:
  ...
  for tid in ids:c.execute("INSERT INTO contact_tags(contact_id,tag_id,created_at) VALUES(?,?,?)",(cid,tid,t))
...
def admin_patch_contact(db,wid,cid,p):
 ...
 with db.tx() as c:
  ...
  for key,value in custom.items():
   ...
   if mismatch:return err('FIELD_TYPE_MISMATCH','自定义字段类型不匹配',400,fieldKey=key,expectedType=typ)
   c.execute("INSERT INTO contact_custom_values(contact_id,field_key,value_json,updated_at,updated_by) VALUES(?,?,?,?,?) ON CONFLICT(contact_id,field_key) DO UPDATE SET value_json=excluded.value_json,updated_at=excluded.updated_at,updated_by=excluded.updated_by",(cid,key,dumps(value),t,actor))
...
def admin_overview(db,wid='default'):
 with db.tx(False) as c:
  ...
  counts['active_contacts']=int(c.execute("SELECT COUNT(*) FROM contacts WHERE workspace_id=? AND deleted_at IS NULL",(wid,)).fetchone()[0])
  counts['deleted_contacts']=int(c.execute("SELECT COUNT(*) FROM contacts WHERE workspace_id=? AND deleted_at IS NOT NULL",(wid,)).fetchone()[0])
  counts['rejected_events']=int(c.execute("SELECT COUNT(*) FROM events WHERE workspace_id=? AND status='rejected'",(wid,)).fetchone()[0])
...
def admin_schema(db):
 with db.tx(False) as c:
  ...
  for r in c.execute("SELECT name,sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name"):
   cols=[dict(x) for x in c.execute('PRAGMA table_info('+r['name']+')')]
   fks=[dict(x) for x in c.execute('PRAGMA foreign_key_list('+r['name']+')')]
   out.append({'name':r['name'],'sql':r['sql'],'columns':cols,'foreignKeys':fks})
  return ok(items=out)

# V207.3.0 兼容层更新说明（2026-09-19）：Schema 207 适配；
# 同步补齐 devices/sources/contacts/audit_logs 的多租户与设备双命名列；
# 公开函数 schemaVersion 统一为 207；保持 V205/V206 事件协议字节级兼容。
